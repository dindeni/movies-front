...
describe('DailyTop', () => {
  ...
  it('should render slider', async () => {
    ...
    const sliderWrapper = container.querySelector('.swiper-wrapper') as HTMLElement;
    await waitFor(() => expect(sliderWrapper.children.length).toBe(4));
  });

  it('should number tooltip(inside image slide has styling)', async () => {
    const { getByText } = renderWithProviders(component);
    await waitFor(() => expect(getByText('1')).toHaveStyle('border: 1px solid #fed530;'));
  });
  ...
  it('should not have slides when response error occurred', async () => {
    ...
    const { container } = renderWithProviders(component);
    const sliderWrapper = container.querySelector('.swiper-wrapper') as HTMLElement;
    await waitFor(() => expect(sliderWrapper.children.length).toBe(0));
  });
});
